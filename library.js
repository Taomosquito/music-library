const library = {
  ...
  printPlaylist: function(playlistId) {
    ...
    for (let i = 0; i < tracksLength; i++) {
      ...
      const objTrackItemName = this["tracks"][trackItem]["name"];
      const objTrackItemArtist = this["tracks"][trackItem]["artist"];
      const objTrackItemAlbum = this["tracks"][trackItem]["album"];
      console.log(`${trackItem}: ${objTrackItemName} by ${objTrackItemArtist} (${objTrackItemAlbum})`);
    }
  },
  // adds an existing track to an existing playlist
  addTrackToPlaylist: function(trackId, playlistId) {
    const track = this["tracks"][trackId]["id"];
    const playlist = this["playlists"][playlistId];
  ...
  addTrack: function(name, artist, album) {
    const fastTracks = this["tracks"];
    newId = this.generateUid();
    fastTracks[newId] = {
      id: newId,
      name: name,
      artist: artist,
